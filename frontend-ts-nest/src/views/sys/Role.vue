...
export default class Role extends Vue {
    ...
    submitForm(formName: string) {
        ...
        refs.validate((valid: any) => {
            if (valid) {
                axios.post('/api/sys/role/' + (this.editForm.id ? 'update' : 'create'), this.editForm)
                    .then(res => {
                        this.getRoleList()
                        this.$notify({
                            title: '',
    ...
    editHandle(id: number) {
        axios.get(`/api/sys/role/info/${id}`).then((res: any) => {
            this.editForm = res
    ...
    permHandle(id: number) {
        ...
        axios.get(`/api/sys/role/menu/handle/${id}`).then((res: any) => {
            const permTree: any = this.$refs.permTree
            permTree.setCheckedKeys(res.menuIds)
            this.permForm = res
        })
    }
    ...
    submitPermFormHandle(formName: string) {
        ...
        axios.post(`api/sys/role/menu/handle/update/${this.permForm.id}`, menuIds).then(res => {
            this.getRoleList()
            this.$notify({
                title: '',
                showClose: true,
